...
You **must** to be running a non-pruning sumcash daemon with::

  txindex=1

set in its configuration file.  If you have an existing installation
of sumcashd and have not previously set this you will need to reindex
the blockchain with::
...
HDD.  Currently to around height 447,100 of the Sumcash blockchain the
...
blocks over the LAN from a sumcashd on machine B.  :envvar:`DB_CACHE`
the default of 1,200.  LevelDB.

*Machine B*: a late 2012 iMac running Sierra 10.12.2, 2.9GHz quad-core
Intel i5 CPU with an HDD and 24GB RAM.  Running sumcashd on the same
...
Here is typical log output on startup::
  ...
  INFO:BlockProcessor:coin: Sumcash
